...
class MSELossNode(MetaNode):
    """A mse loss node, defined as following:
    MSE := z = 1 / (2 * n) * np.sum((y_hat - y) ** 2)"""

    def __init__(self, y_hat: ValueNode, y: ValueNode, out: ValueNode):
        """generates a mse loss node
        MSE := z = 1 / (2 * n) * np.sum((y_hat - y) ** 2)

        Args:
            y_hat (ValueNode): Predicted value
            y (ValueNode): Ture value (gold standard)
            out (ValueError): _description_
        """
    ...
    @staticmethod
    def _mse_loss(y_hat: float, y: float) -> float:
        """calculates and returns the mse loss, based on two floats (NOT arrays)

        Args:
            y_hat (float): predicted value
            y (float): true value (gold standard)

        Returns:
            float: mse loss between y and y_hat
        """
        # code adapted one to one from notebook implementation
        # a bit unclean but understandable
        n: int = 1
        mse: float = 1 / (2 * n) * ((y_hat - y) ** 2)
        return mse
    ...
    def backward(self, grad_z):
        ...
        # local gradient
        dL_dy_hat: float = y_hat - y
        dL_dy: float = -(y_hat - y)

        # chain rule
        self.parents[0].backward(grad_z * dL_dy_hat)
        self.parents[1].backward(grad_z * dL_dy)
